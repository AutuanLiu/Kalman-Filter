...
class Selector:
    """基于 FROLS 算法的候选项选择器算法(通用算法，无关线性或者非线性), 基于文件路径
    ...
    def __init__(self, terms_path_or_data):
        """基于 FROLS 算法的候选项选择器算法

        Args:
            terms_path_or_data (str or dict): term selector(matlab) 程序的结果路径 或者 程序的结果
        """

        for key in ['normalized_signals', 'Hv', 'Kalman_H', 'terms_chosen']:
            if isinstance(terms_path_or_data, str):
                setattr(self, key, get_mat_data(terms_path_or_data, key))
            elif isinstance(terms_path_or_data, dict):
                setattr(self, key, terms_path_or_data[key])
            else:
    ...
    def make_selection(self):
        """生成与模型候选项选择有关的数据执行此方法前，先执行 make_terms 方法如果需要计算模型表达式
